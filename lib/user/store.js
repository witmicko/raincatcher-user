...
Store.prototype.listen = function(topicPrefix, mediator) {
  ...
  this.topics
  .on('username:read',  function(data) {
    self.updatePassword(data.username, data.oldPwd, data.newPwd).then(function(user) {
      mediator.publish([self.topics.getTopic('username:read', 'done'), data.username].join(':'), user);
    }).catch(function(err) {
      mediator.publish([self.topics.getTopic('username:read', 'error'), data.username].join(':'), err.message);
    });
  })
